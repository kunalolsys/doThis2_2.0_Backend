  ...
  try {
    ...
    for (const instance of instances) {
      ...
      for (const task of tasks) {
        ...
        if (await FmsInstanceTask.findOne({
        ...
        const shiftStart = await nextWorkingShiftDate(new Date(), user.assignShift._id);
        ...
        createdCount++;
        console.log(`✅ ${task.taskId} | ${format(shiftStart, 'HH:mm dd-MM')}→${format(shiftEnd, 'HH:mm')}`);
      }
    }
...
const startRecurringFmsTaskJob = () => {
  ...
  // cron.schedule('0 1 * * *', generateRecurringFmsTasks, { timezone: "Asia/Kolkata" });
};
