...
const makeTasksVisible = async () => {
  ...
  try {
    ...
    console.log(`📊 [SUMMARY] Processed ${processedUsers}/${usersWithShifts.length} users | Updated ${updatedCount} tasks | ${new Date().toISOString()}`);
  } catch (error) {
...
const hideCompletedShiftTasks = async () => {
  console.log("🔒 [TASK VISIBILITY CRON] Hiding overnight tasks...", { timestamp: new Date().toISOString() });
  ...
  try {
    ...
    for (const user of usersWithShifts) {
      ...
      console.log(`🔍 ${user.name} shift end: ${shiftEndToday.toLocaleTimeString()} | now: ${now.toLocaleTimeString()}`);
